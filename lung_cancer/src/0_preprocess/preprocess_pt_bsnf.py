...
#%%
bsnf_required['BSPT_AGE'] = (bsnf_required['BSPT_FRST_DIAG_YMD'] - bsnf_required['BSPT_BRYM'])
bsnf_required['BSPT_AGE'] = bsnf_required['BSPT_AGE'].dt.days // 365

#%%
bsnf_required = bsnf_required.drop(columns='BSPT_BRYM')

#%%
bsnf_required = bsnf_required.rename(columns ={'BSPT_FRST_DIAG_YMD':"TIME"}).set_index(['PT_SBST_NO','TIME'])

#%%
bsnf_required['BSPT_FRST_DIAG_YMD'] = 1

#%%
bsnf_final = bsnf_required.copy()
#%%
duplicated_counts = bsnf_final.reset_index().duplicated(subset=['PT_SBST_NO','TIME']).sum()
...
#%%
bsnf_final.to_pickle(output_path.joinpath('lung_pt_bsnf.pkl'))
# %%
