...
fill = all_concated.filter(regex="EX_PLMN|PT_SBST_NO").groupby(['PT_SBST_NO'],as_index=False).transform(lambda v : v.ffill())
msk = all_concated.columns.str.contains('EX_PLMN_')
...
#%%
all_concated = all_concated.fillna({'DEAD_NFRM_DEAD':0})

#%%
change2objectdtype = all_concated.filter(regex='DEAD_NFRM').columns.tolist()
