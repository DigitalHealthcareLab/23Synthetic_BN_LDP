...
def return_description_files(path : Path):
    ...
    f = (lambda x : x.split('_')[1].split('.')[0])
...
def main():
    ...
    parser.add_argument('--sample_number', '-sn', default=0, type = int, help='how much you are going to sample')
    ...
    #description_idx = list(map(return_description_files, files)) * args['multiplier']
    description_idx = files * args.multiplier
    pseudo_patient_id = [i for i in range(0, args.multiplier*len(files))]
    
    
    with _ProcessPool(8) as p:
        p.starmap(generate_data, zip(repeat(args.epsilon), description_idx, pseudo_patient_id))
