...
INPUT_PATH = PROJ_PATH.joinpath('/mnt/synthetic_data/lung/data/processed/2_restore/restore_to_db_form')
OUTPUT_PATH = PROJ_PATH.joinpath('/mnt/synthetic_data/lung/data/processed/4_evaluate/make_death')
...
def load_s1(epsilon):
    return read_file(PROJ_PATH.joinpath(f'/mnt/synthetic_data/lung/data/processed/2_restore/restore_to_s1'), f'S1_{epsilon}.pkl')

#%%

# data = load_file_epsilon(1)
# original_data =pd.read_excel(PROJ_PATH.joinpath('data/raw/LUNG_PT_BSNF.xlsx'))
# s1 = load_s1(1)
...
def main():
    ...
    #%%
    operation_data = s1.filter(regex='OPRT|TIME|PT_SBST_NO').dropna(subset=['OPRT_NFRM_OPRT_LUCN_OPRT_KIND_CD'], how='all')
    ...
    #%%
    rd_data = s1.filter(regex='TRTM_RD|TIME|PT_SBST_NO').dropna(subset=['TRTM_RD_RDT_STRT_YMD'], how='all')
    ...
    # %%
    comparison_data = data[['PT_SBST_NO','BSPT_SEX_CD','BSPT_FRST_DIAG_CD','BSPT_AGE','BSPT_STAG_VL','BSPT_T_STAG_VL','BSPT_N_STAG_VL','BSPT_M_STAG_VL','BSPT_OPRT','TRTM_RD_RDT','OVRL_DAYS','DEAD']].copy()
    ...
    ori = ori.rename(columns = {"OVRL_SRVL_DTRN_DCNT":"OVRL_DAYS",
                                "BSPT_IDGN_AGE" : "BSPT_AGE"})

    ori = ori[['PT_SBST_NO','BSPT_SEX_CD','BSPT_FRST_DIAG_CD','BSPT_AGE','BSPT_STAG_VL','BSPT_T_STAG_VL','BSPT_N_STAG_VL','BSPT_M_STAG_VL','BSPT_OPRT','TRTM_RD_RDT','OVRL_DAYS','DEAD']].copy()
