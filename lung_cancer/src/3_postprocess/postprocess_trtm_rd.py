...
#%%
    ...
    data = data.dropna(subset='RDT_STRT_YMD')

    data = data.drop('RDT_STRT_YMD',axis=1)
    data = data.rename(columns = {'TIME':'RDT_STRT_YMD'})
    data['RDT_END_YMD'] = data['RDT_STRT_YMD'] + pd.to_timedelta(25, 'days') # need to check quantity of additonal
    ...
    data['RDT_RT_NO'] = data.groupby('PT_SBST_NO').cumcount()+1
