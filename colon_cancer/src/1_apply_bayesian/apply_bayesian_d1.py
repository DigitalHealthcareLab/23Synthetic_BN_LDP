...
INPUT_PATH = PROJ_PATH.joinpath('/mnt/synthetic_data/data/processed/1_apply_bayesian/preprocess_data/D0_train')
OUTPUT_PATH = PROJ_PATH.joinpath('/mnt/synthetic_data/data/processed/1_apply_bayesian/apply_bayesian/D0_train')
...
def main():
    ...
    data = read_file(Path('/mnt/synthetic_data/'), 'data/processed/0_preprocess/D0_train.pkl')
    patients = data['PT_SBST_NO'].unique().tolist()
    #epsilons  = config['epsilon']

    parser = argparse.ArgumentParser()
    parser.add_argument('--epsilon','--e',type=int,help='epsilon values')
    args = parser.parse_args()
    epsilon = args.epsilon
    #random.seed(config['random_seed'])
    #sampled_patients = random.sample(patients, 100)
    with _ProcessPool(2) as p :
