...
#%%
    ...
    original_path =PROJ_PATH.joinpath(f'/mnt/synthetic_data/data/raw/{file_name}.csv')
    ...
    #%%
    data_melted['CEXM_YMD']= data_melted['CEXM_YMD'].astype('datetime64[ns]')
    data_melted = data_melted.astype(dtypes)

    data_melted = data_melted.drop_duplicates(subset=['PT_SBST_NO','CEXM_RSLT_CONT'])
    data_melted = data_melted.dropna(subset=['CEXM_RSLT_CONT'])
    data_melted.to_csv(OUTPUT_PATH.joinpath(f'{file_name}_{args.epsilon}.csv'))
    pass
