...
def main():
    ...
    data = data.query('RLPS == 1').reset_index(drop=True)
    
    data = data.drop(columns = "RLPS")

    data = data.rename(columns={'TIME':"RLPS_DIAG_YMD"})
    ...
    data['RLPS_DIAG_YMD']= data['RLPS_DIAG_YMD'].astype('datetime64[ns]')
